...
def technical_quality(model, video_list, device, **kwargs):
    ...
    for video_path in tqdm(video_list, disable=get_rank() > 0):
        ...
        for i in range(len(images)):
            # SmartDiffusion generation may run under autocast/bf16; MUSIQ conv expects
            # input/weights to share dtype. Force fp32 here for stability.
            frame = images[i].unsqueeze(0).to(device=device, dtype=torch.float32)
            with torch.no_grad():
