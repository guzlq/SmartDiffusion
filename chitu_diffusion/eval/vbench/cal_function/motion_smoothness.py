...
class MotionSmoothness:
    ...
    def load_model(self):
        ...
        self.model = self.model.to(self.device).float()  # 强制转成 float32 避免 dtype 冲突
        self.model.eval()
    ...
    def motion_score(self, video_path):
        ...
        for i in range(iters):
            ...
            for in_0, in_1 in zip(inputs[:-1], inputs[1:]):
                in_0 = in_0.to(self.device, dtype=torch.float32)  # 强制 float32
                in_1 = in_1.to(self.device, dtype=torch.float32)  # 强制 float32
                with torch.no_grad():
                    # 关闭 autocast 避免 dtype 冲突
                    with torch.cuda.amp.autocast(enabled=False):
                        imgt_pred = self.model(in_0, in_1, self.embt, scale_factor=scale, eval=True)['imgt_pred']
                # 确保输出是 float32，避免 bfloat16 无法转 numpy
                outputs += [imgt_pred.cpu().float(), in_1.cpu().float()]
            inputs = outputs

        # -----------------------  cal_vfi_score ----------------------- 
        outputs = padder.unpad(*outputs)
        # 确保所有 tensor 都是 float32，避免 bfloat16 无法转 numpy
        outputs = [out.float() if isinstance(out, torch.Tensor) else out for out in outputs]
