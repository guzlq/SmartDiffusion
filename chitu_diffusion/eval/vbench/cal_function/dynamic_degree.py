...
class DynamicDegree:
    ...
    def load_model(self):
        ...
        # Force fp32 for stability (avoid bf16/amp dtype mismatch inside RAFT convs)
    ...
    def infer(self, video_path):
        with torch.no_grad():
            ...
            for image1, image2 in zip(frames[:-1], frames[1:]):
                ...
                # SmartDiffusion may enable autocast/bf16 globally; RAFT expects input/weights
                # dtype to match. Force fp32 inference here.
                image1 = image1.to(dtype=torch.float32)
                image2 = image2.to(dtype=torch.float32)
