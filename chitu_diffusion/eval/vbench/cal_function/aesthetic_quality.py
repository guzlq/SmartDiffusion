import os
# 确保导入 OpenAI CLIP（避免与本地 clip.py 冲突）
import sys
...
def compute_aesthetic_quality(json_dir, device, submodules_list, **kwargs):
    ...
    logger.info(f"[Rank {get_rank()}] Loading aesthetic model...")
    if get_rank() == 0:
        aesthetic_model = get_aesthetic_model(aes_path).to(device).float()
        barrier()
    else:
        barrier()
        aesthetic_model = get_aesthetic_model(aes_path).to(device).float()
    logger.info(f"[Rank {get_rank()}] Loading CLIP model...")
    clip_model, preprocess = clip.load(vit_path, device=device)
    clip_model = clip_model.float()
    logger.info(f"[Rank {get_rank()}] Loading video list...")
    video_list, _ = load_dimension_info(json_dir, dimension='aesthetic_quality', lang='en')
    video_list = distribute_list_to_rank(video_list)
    logger.info(f"[Rank {get_rank()}] Processing {len(video_list)} videos...")
    all_results, video_results = laion_aesthetic(aesthetic_model, clip_model, video_list, device)
    logger.info(f"[Rank {get_rank()}] Finished processing, starting gather...")
    if get_world_size() > 1:
        video_results = gather_list_of_dict(video_results)
        logger.info(f"[Rank {get_rank()}] Gather completed")
        all_results = sum([d['video_results'] for d in video_results]) / len(video_results)
    logger.info(f"[Rank {get_rank()}] aesthetic_quality completed")
