...
def init_submodules(dimension_list, local=False, read_frame=False):
    ...
    for dimension in dimension_list:
        ...
        if dimension == 'background_consistency':
            ...
            # SmartDiffusion adaptation:
            # third_party is a sibling of cal_function (../third_party), not under cal_function/.
            CUR_DIR = os.path.dirname(os.path.abspath(__file__))
            VBENCH_DIR = os.path.dirname(CUR_DIR)
            submodules_dict[dimension] = {
                    'config': f'{VBENCH_DIR}/third_party/amt/cfgs/AMT-S.yaml',
